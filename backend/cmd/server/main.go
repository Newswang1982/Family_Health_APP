...
func main() {
	...
	qrHandler := handler.NewQRLoginHandler(cfg.JWTSecret)
	...
	auth := r.Group("/api/v1/auth")
	{
		...
		// QR Code Login
		auth.GET("/qr/generate", qrHandler.GenerateQRToken)
		auth.GET("/qr/status/:token", qrHandler.CheckQRStatus)
	...
	api.Use(middleware.AuthMiddleware(cfg.JWTSecret))
	{
		...
		api.POST("/auth/qr/scan", qrHandler.ScanQR)
		api.GET("/auth/qr/user-info", qrHandler.GetUserInfo)
